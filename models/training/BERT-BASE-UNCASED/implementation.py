...
training_samples = torch.load('././tokenized/BERT-BASE-UNCASED/train.pth') # Call the tokenized dataset
...
############################################## validation ########################################################################
validation_samples = torch.load('././tokenized/BERT-BASE-UNCASED/validation.pth') # Call the tokenized dataset
...
def load_model (model_name = 'bert-base-uncased', num_labels=7, use_FocalLoss = False, alpha=.65, gamma=4, weight_decay=.01, hidden_dropout=.3, attention_dropout = .35):
    ...
    for batch_id, batch in enumerate(progress):
        ...
        if batch_id != 0 and batch_id % evaluate == 0:
            ...
            if val_loss_mean < BEST_VAL_LOSS:
                ...
                NO_IMPROVEMENT = 0
                torch.save(model.state_dict(), f'./saved/{model_name}/checkPoints/Best-BERT-UNCASED-Checkpoint.pt')
            else :
                NO_IMPROVEMENT += 1
...
try:
    ...
    for epoch in progress:
        ...
        torch.save(model.state_dict(), f'./saved/{model_name}/checkPoints/model_bert_based_uncased_epoch_{epoch}.pt')
        
    with open('./Metrics_results/BERT-Base-Uncased/BERT_Based_Uncased_cycleLR_training.pkl', 'wb') as f:
        pickle.dump(train_loss_results, f)
        
    with open('./Metrics_results/BERT-Base-Uncased/BERT_Based_Uncased_cycleLR_learning_rates.pkl', 'wb') as f:
        pickle.dump(learning_rates, f)

    with open('./Metrics_results/BERT-Base-Uncased/BERT_Based_Uncased_cycleLR_validation.pkl', 'wb') as f:

...
training_samples = pd.read_csv('././Data/multi-lingual/hs_train.csv') #torch.load('././tokenized/DistilBERT-BASE-CASED/train.pth') # Call the tokenized dataset
training_dataset = BinaryHateCrimeDataset(training_samples, model_name=MODEL_NAME, max_length=MAX_LENGTH, has_labels=True) # convert it to a pytorch dataset
...
############################################## validation ########################################################################
validation_samples = pd.read_csv('././Data/multi-lingual/hs_val.csv') #torch.load('././tokenized/DistilBERT-BASE-CASED/validation.pth') # Call the tokenized dataset
validation_dataset = BinaryHateCrimeDataset(validation_samples, model_name=MODEL_NAME, max_length=MAX_LENGTH, has_labels=True) # convert it to a pytorch dataset
...
checkpoint_callback = ModelCheckpoint(
    ...
    dirpath='./saved/mBERTu',  # Directory to save the checkpoints
    filename='mBERTu-FL-{epoch:02d}-{val_loss:.6f}',  # Filename pattern

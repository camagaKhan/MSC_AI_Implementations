...
class LightHateCrimeModel (LightningModule) :
    ...
    def __init__(self, model_name='distilbert-base-cased', step_size_up = 1000, metric_threshold = .5, binary_base_lr = 1e-7,  binary_lr=2e-5, batch_size=16, attn_dropout = .1 , hidden_dropout = .1, model_dropout = .1, use_focalLoss = False, alpha = .25, gamma = 2., n_bins = 10):
    ...
    def forward(self, input_ids, attn_mask, apply_temperature = False):
    ...
    def on_train_epoch_end(self): 
        ...
        with open(f'./Metrics_results/{self.model_name}/training/{self.model_name}_training_{epoch_id}_scores_{uuid.uuid4()}.pkl', 'wb') as f:
            pickle.dump(b_metrics, f)              
            
        with open(f'./Metrics_results/{self.model_name}/validation/{self.model_name}_validation_{epoch_id}_scores_{uuid.uuid4()}.pkl', 'wb') as f:
    ...
    def validation_step(self, validation_batch, batch_idx):
        ...
        outputs = self(token_list_batch, attention_mask_batch, apply_temperature = True)
        binary_logits = outputs.squeeze()
        loss = self.loss_fn_binary(binary_logits.to(torch.float32), label_batch)

...
LR = 4e-5 # 2e-5 #1e-5 <-- Learning rate is ok # Initial Learning Rate ; 5e-5 was bad
EPOCHS = 3
num_labels = 4 # removed severe_toxicity
...
training_samples = torch.load('././tokenized/DistilBERT-BASE-CASED/multi-label/train_minority.pth') # Call the tokenized dataset
...
############################################## validation ########################################################################
validation_samples = torch.load('././tokenized/DistilBERT-BASE-CASED/multi-label/validation_minority.pth') # Call the tokenized dataset
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='multi-label-distilbert-FL-minority-cased-{epoch:02d}-{val_loss:.6f}',  # Filename pattern
...
trainer = Trainer(
    val_check_interval=400, # run validation loop every 5000 steps
    max_epochs=EPOCHS, 

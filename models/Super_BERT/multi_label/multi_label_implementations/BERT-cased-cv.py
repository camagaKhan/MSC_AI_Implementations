...
try:
    ...
    for fold, (train_index, val_index) in enumerate(
        ...
        for epoch in progress:
            ...
            val_average_loss, val_metrics_computed = validate_epoch(transformer=transformer, fold=fold+1, criterion=criterion, validation_dl=validation_dataloader, device=device, validation_metric=validation_metric, epoch=epoch)    
             # save checkpoint
            torch.save(transformer, f'./saved/bert-base-cased/fold/BERT_FL_{BATCH}_{epoch}_fold_{fold+1}.model')    
    ...
    with open(f'././././Metrics_results/bert-base-cased/training/BERT-cased-kfold-FocalLoss_{BATCH}_training_{LEARNING_RATE}.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open(f'././././Metrics_results/bert-base-cased/validation/BERT-cased-kfold-FocalLoss_{BATCH}_validation_{LEARNING_RATE}.pkl', 'wb') as f:

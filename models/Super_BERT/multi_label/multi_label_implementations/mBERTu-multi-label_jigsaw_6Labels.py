...
training_pd, validation_pd = pd.read_csv('././Data/multi-lingual/hs_train.csv'), pd.read_csv('././Data/multi-lingual/hs_val.csv')
...
transformer = HateSpeechTagger(model_name=MODEL_NAME, n_classes=NUM_LABELS, attn_dropout=.4, model_dropout=.3) # model_dropout was .3
...
warmup_steps = int(0.5 * total_steps) # used to be one
...
def validate_epoch(epoch):
    ...
    #     print(f"Validation loss improved from {best_validation_loss} to {avg_validation_loss}. Saving model...")
...
try:
    ...
    for epoch in progress:
       ...
       torch.save(transformer, f'././././saved/mBERTu/BERTu_jigsaw_{loss_fn_name}_{epoch}_6lbls_jigsaw.model')
       
       torch.cuda.empty_cache() # always empty cache before you start a new epoch
    
    with open(f'././././Metrics_results/mBERTu/training/BERTu-ML-Cased-jigsaw_6lbls_{loss_fn_name}__training.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open(f'././././Metrics_results/mBERTu/validation/BERTu-ML-Cased-jigsaw_6lbls_{loss_fn_name}__validation.pkl', 'wb') as f:

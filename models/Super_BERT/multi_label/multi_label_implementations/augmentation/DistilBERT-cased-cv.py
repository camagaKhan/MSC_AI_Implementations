...
if os.path.isdir('./././././Metrics_results/'):
    print("Directory exists")
else:
    print("Directory does not exist")
...
checkpoint_dir = "./saved/distilbert-base-cased"
...
try:
    ...
    for fold, (train_index, val_index) in enumerate(
        ...
        for epoch in progress:
            ...
            val_average_loss, val_metrics_computed = validate_epoch(transformer=transformer, fold=fold+1, criterion=criterion, validation_dl=validation_dataloader, device=device, validation_metric=validation_metric, epoch=epoch)    
             # save checkpoint
            torch.save(transformer, f'./././././saved/distilbert-base-cased/fold/BERT_FL_{BATCH}_{epoch}_fold_{fold+1}.model')    
    ...
    with open(f'./././././Metrics_results/distilbert-base-cased/training/DistilBERT-kfold-FocalLoss_{BATCH}_augmentation_training_{LEARNING_RATE}.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open(f'./././././Metrics_results/distilbert-base-cased/validation/DistilBERT-kfold-FocalLoss_{BATCH}_augmentation_validation_{LEARNING_RATE}.pkl', 'wb') as f:

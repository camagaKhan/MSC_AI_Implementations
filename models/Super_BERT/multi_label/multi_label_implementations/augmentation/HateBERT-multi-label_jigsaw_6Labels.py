...
import tqdm
import sys 
from torchmetrics import ConfusionMatrix, MetricCollection, Metric, Accuracy, Precision, Recall, AUROC, F1Score, ROC, PrecisionRecallCurve
import pickle
sys.path.append('./')
...
try:
    ...
    for epoch in progress:
       # Start training
       train_epoch(epoch=epoch, check_interval=8_000)
       
       # validate epoch
       validate_epoch(epoch=epoch)
       
       torch.save(transformer, f'./././././saved/hateBERT/HateBERT_augmented_{MAX_LENGTH}_{loss_fn_name}_{epoch}_6lbls_jigsaw_{LEARNING_RATE}.model')
       
       torch.cuda.empty_cache() # always empty cache before you start a new epoch
    
    with open(f'./././././Metrics_results/HateBert/training/HateBERT_augmented_{MAX_LENGTH}_{loss_fn_name}_jigsaw_6lbls_training_{LEARNING_RATE}.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open(f'./././././Metrics_results/HateBert/validation/HateBERT_augmented_{MAX_LENGTH}_{loss_fn_name}_jigsaw_6lbls_validation_{LEARNING_RATE}.pkl', 'wb') as f:

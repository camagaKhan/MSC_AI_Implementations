...
from model_skeleton_multilabel_v4 import HateSpeechv2Dataset, HateSpeechTagger
...
#train = torch.load(f'././././tokenized/{tokenizer_folder_name}/multi-label/train_cv.pth')
...
try:
    ...
    for fold, (train_index, val_index) in enumerate(
        ...
        # set the optimizer
        optimizer = torch.optim.AdamW(params=transformer.parameters(), lr=LEARNING_RATE)
        ...
        for epoch in progress:
            ...
            val_average_loss, val_metrics_computed = validate_epoch(transformer=transformer, fold=fold+1, criterion=criterion, validation_dl=validation_dataloader, device=device, validation_metric=validation_metric, epoch=epoch)    
             # save checkpoint
            torch.save(transformer, f'./saved/hateBERT/fold/hateBERT_FL_{epoch}_fold_{fold+1}.model')    
    ...
    with open(f'././././Metrics_results/HateBert/training/HateBert-kfold-FocalLoss_training_{LEARNING_RATE}.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open(f'././././Metrics_results/HateBert/validation/HateBert-kfold-FocalLoss_validation_{LEARNING_RATE}.pkl', 'wb') as f:

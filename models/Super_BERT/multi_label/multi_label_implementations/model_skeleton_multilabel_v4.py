...
class HateSpeechEnsembleDataset (Dataset): # For Ensemble
    def __init__(self, dataset, model_name_1, model_name_2, max_length = 128, multi_label = True, forTraining = True, without_sexual_explict = True) -> None:
        super().__init__()
        self.data = dataset
        print(len(self.data))
        self.has_labels = forTraining
        self.model_name1 = model_name_1
        self.model_name2 = model_name_2
        self.max_length = max_length
        
        self.tokenizer1 = AutoTokenizer.from_pretrained(self.model_name1)
        self.tokenizer2 = AutoTokenizer.from_pretrained(self.model_name2)
    ...
    def __getitem__(self, index):
        ...
        tokens_model_1 = self.tokenizer1(item['comment_text'], add_special_tokens=True, max_length=self.max_length, padding='max_length', truncation = True, return_tensors='pt')
        tokens_model_2 = self.tokenizer2(item['comment_text'], add_special_tokens=True, max_length=self.max_length, padding='max_length', truncation = True, return_tensors='pt')
        labels = torch.tensor(item[self.targets].astype(float).values, dtype=torch.float)
        
        if self.has_labels :
            dictionary = dict(
                index = index,
                input_ids_1 = tokens_model_1['input_ids'].squeeze(),
                attention_mask_1 = tokens_model_1['attention_mask'].squeeze(),
                input_ids_2 = tokens_model_2['input_ids'].squeeze(),
                attention_mask_2 = tokens_model_2['attention_mask'].squeeze(),
                labels = labels
            )
        else : 
            dictionary = dict(
...
class HateSpeechv2Dataset (Dataset):
    def __init__(self, dataset, model_name, max_length = 128, multi_label = True, forTraining = True, without_sexual_explict = True) -> None:
        super().__init__()
        self.data = dataset
        print(len(self.data))
        self.has_labels = forTraining
...
class HateSpeechTagger(nn.Module) :    
    ...
    def forward (self, input_ids, attention_mask):
        ...
        # get probabilities
        logits = self.classify(output) # last hidden states is used for classification.         

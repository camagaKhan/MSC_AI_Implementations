...
binary_labels = train['labels'].to(torch.int32).numpy()
...
for i in range(binary_labels.shape[1]):
    ...
    weight = class_weight.compute_class_weight(class_weight='balanced',
                                        ...
                                        y=y)
    class_weights.append(weight[0])  # weight[0] corresponds to the weight for class '1'
...
criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weights)  #FocalLoss(alpha=.25, gamma=3) #torch.nn.BCELoss() # <-- unlike bce logits loss, this loss function's result will not be influenced by an internal sigmoid function. I might revise FocalLoss

THRESHOLD, num_labels = .3, 6
...
try:
    ...
    for epoch in progress:
       # Start training
       train_epoch(epoch=epoch, check_interval=1_500)
       
       # validate epoch
       validate_epoch(epoch=epoch)
       
       torch.save(transformer, f'./saved/distilbert-base-cased/fold/BERT_FL_{epoch}_wiki.model')
       
       torch.cuda.empty_cache() # always empty cache before you start a new epoch
    
    with open('././././Metrics_results/distilbert-base-cased/training/DistilBERT-wiki_training.pkl', 'wb') as f:
        pickle.dump(training_log, f)

    with open('././././Metrics_results/distilbert-base-cased/validation/DistilBERT-wiki_validation.pkl', 'wb') as f:

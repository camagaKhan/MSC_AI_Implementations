...
test_dataset = BinaryHateCrimeDataset(testing_samples, model_name=MODEL_NAME, max_length=MAX_LENGTH, has_labels=True, label_name='blacklist') # convert it to a pytorch dataset

test_dataloader = DataLoader(test_dataset, batch_size=BATCH_SIZE, num_workers=0, pin_memory=True, shuffle=False)

folder_name, file_name = 'distilbert-base-cased',  'blacklist-distilbert-FL-epoch=00-val_loss=0.011879'
...
with open('././Metrics_results/distilbert-base-cased/test/blacklist-wiki_labels-DistilBERT-binary-FocalLoss_test.pkl', 'wb') as f:

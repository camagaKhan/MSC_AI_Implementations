import torch 
import sys 
from torchmetrics import MetricCollection, Accuracy, ConfusionMatrix, Precision, Recall, AUROC, F1Score, ROC, PrecisionRecallCurve
...
testing_samples = pd.read_csv('././Data/wiki_labels_with_not_offensive.csv') # Call the tokenized dataset
#testing_samples['blacklist'] = 1 
#testing_samples.loc[testing_samples['not_offensive'] == 1, 'blacklist'] = 0
test_dataset = BinaryHateCrimeDataset(testing_samples, model_name=MODEL_NAME, max_length=MAX_LENGTH, has_labels=True, label_name='not_offensive') # convert it to a pytorch dataset

test_dataloader = DataLoader(test_dataset, batch_size=BATCH_SIZE, num_workers=0, pin_memory=True, shuffle=False)

folder_name, file_name = 'distilbert-base-cased',  'not-offensive-distilbert-FL-epoch=01-val_loss=0.011651'
...
base_model = LightHateCrimeModel(model_name=MODEL_NAME, metrics_folder_name='distilbert-base-cased')
...
with open('././Metrics_results/distilbert-base-cased/test/not-offensive-DistilBERT-binary-FocalLoss_test.pkl', 'wb') as f:

...
if os.path.isdir('./././saved/hateBERT/fold/'):
...
for i in range(1, 6):
    ...
    model = torch.load(f'./././saved/hateBERT/fold/{checkpoint}')
    model.to(device)
    model.eval()
    models.append(model)
...
with open(f'././././Metrics_results/ensemble/HateBert-ML-Cased-jigsaw_6lbls_{'FL'}_ensemble_averaging_training.pkl', 'wb') as f:

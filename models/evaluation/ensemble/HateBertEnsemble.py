...
import pandas as pd
sys.path.append('./')
from models.Super_BERT.multi_label.multi_label_implementations.model_skeleton_multilabel_v4 import HateSpeechv2Dataset
import pickle
import os
import glob 
...
def get_files_with_extension(directory, extension):
    # Use os.path.join to create a search pattern for files with the given extension
    search_pattern = os.path.join(directory, f"*.{extension}")
    # glob.glob will return a list of files matching the pattern
    return glob.glob(search_pattern)

folder_name = 'hateBERT'
# checkpoint_path = f'././saved/{folder_name}/{file_name}.model'

# Example usage:
directory = f'.\\.\\saved\\{folder_name}\\fold'
extension = 'model'  # Change to the extension you're looking for
files = get_files_with_extension(directory, extension)
# files =  ['.\\.\\saved\\hateBERT\\fold\\hateBERT_FL_1_fold_1.model', '.\\.\\saved\\hateBERT\\fold\\hateBERT_FL_1_fold_2.model']
...
for checkpoint_path in tqdm.tqdm(files, desc="Evaluating Checkpoints for Ensemble Stacking"): 
    hs_predictions, hs_targets, predictions, targets = [], [], [], []
    ...
    hs_predictions.append(all_predictions)
    hs_targets.append(all_targets)
    
stacked_predictions = torch.mean(torch.stack(hs_predictions), dim=0)#[0]
stacked_targets = torch.cat(hs_targets)
